...
function extendMaterial(cfg: {
  ...
  const defaults = new THREE.MeshStandardMaterial(cfg.material as THREE.MeshStandardMaterialParameters || {});
...
function createGeometry(n: number, width: number, height: number, segments: number) {
  const geo = new THREE.BufferGeometry();
  const verts = new Float32Array(n * (segments + 1) * 2 * 3);
  const idxs = new Uint32Array(n * segments * 2 * 3);
  const uvs = new Float32Array(n * (segments + 1) * 2 * 2);
  let vi = 0, ii = 0, uvi = 0;
  const totalW = n * width + (n - 1) * 0.1;
  const xBase = -totalW / 2;
  for (let i = 0; i < n; i++) {
    const xOff = xBase + i * (width + 0.1);
    const ux = Math.random() * 300, uy = Math.random() * 300;
    for (let j = 0; j <= segments; j++) {
      const y = height * (j / segments - 0.5);
      verts.set([xOff, y, 0, xOff + width, y, 0], vi * 3);
      uvs.set([ux, j / segments + uy, ux + 1, j / segments + uy], uvi);
      if (j < segments) { const a = vi, b = vi + 1, c = vi + 2, d = vi + 3; idxs.set([a, b, c, c, b, d], ii); ii += 6; }
      vi += 2; uvi += 4;
    }
  }
...
const MergedPlanes = forwardRef<THREE.Mesh, { material: THREE.Material; width: number; count: number; height: number }>(
  ({ material, width, count, height }, ref) => {
    ...
    useFrame((_, delta) => {
      ...
      if (u.time) u.time.value += 0.1 * delta;
    });
...
interface BeamsProps {
  ...
  lightColor?: string; speed?: number; noiseIntensity?: number; scale?: number; rotation?: number;
}

function BeamsScene({ beamWidth = 2, beamHeight = 15, beamNumber = 12, lightColor = '#14b8a6', speed = 2, noiseIntensity = 1.75, scale = 0.2, rotation = 0 }: BeamsProps) {
  const material = useMemo(() => extendMaterial({
